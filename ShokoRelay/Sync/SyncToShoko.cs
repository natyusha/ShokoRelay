    ...
    private async Task<PlexWatchedSyncResult> SyncWatchedInternalAsync(bool dryRun, int? sinceHours, bool includeVotes, bool excludeAdmin, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var target in targets)
        {
            ...
            try
            {
                ...
                Logger.Info("WatchedSyncService: fetched {Count} watched episodes (since={Since}) from Plex target {Server}:{Section}", adminEpisodes?.Count ?? 0, minLast, target.ServerUrl, target.SectionId);
            }
            catch (Exception ex)
            {
