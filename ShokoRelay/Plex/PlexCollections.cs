...
public class PlexCollections
{
    ...
    /// <summary>
    /// Updates the sort title of a collection so Plex orders it based on the provided <paramref name="title"/> rather than default sorting rules. Locks the field to avoid being overwritten.
    /// </summary>
    ...
    public async Task<bool> UpdateCollectionTitleSortAsync(int collectionId, string title, PlexLibraryTarget target, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            string requestPath = $"/library/metadata/{collectionId}?titleSort={Uri.EscapeDataString(title)}&titleSort.locked=1";
            using var request = _plexClient.CreateRequest(HttpMethod.Put, requestPath, target.ServerUrl);
            using var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
