...
POST /vfs/subtitles/preview                                    -> PreviewSubtitleRules
...
- `PreviewSubtitleRules` accepts `{"SeriesId": 123, "Rules": [{"OriginalSuffix": "chs", "FinalSuffix": "zh-Hans"}]}`. It reads eligible sidecars for the series and returns `files` with `source`, `suffix`, and `reason`, plus any directory-read `errors`. An empty output suffix means the source is omitted by priority or format selection. The returned suffix includes its leading dot and extension; the VFS video basename is generated by the normal build. Preview rules are validated but are not saved, and no VFS links or blueprint files are written.
